...
if __name__ == '__main__':
    print("Running active defender runner")
    seed_paths = list_file_paths('samples/seeds')
    targeted_seeds = ["994f7552e4715688257141102931796246d15e86"]
    targeted_seed_paths = ['samples/seeds/%s.pdf' %seed for seed in targeted_seeds]
    if len(sys.argv) < 2:
    ...
    if not os.path.isdir(ext_genome_folder):
        ...
        sys.exit(1)
    for seed_path in targeted_seed_paths[:]:
	if  seed_path in seed_paths:
		print("Trying seedpath:  " + str(seed_path))
